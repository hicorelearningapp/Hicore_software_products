...
const CourseContent = ({ selectedCourse = "C001" }) => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const res = await fetch(`${API_BASE}/find-my-courses/module/${courseKey}`);
        console.log("📡 Fetching from:", `${API_BASE}/find-my-courses/module/${courseKey}`);
  ...
  const handleEnrollClick = async () => {
    ...
    try {
      ...
      if (checkData.has_access) {
        ...
        navigate(`/find-my-courses/${courseKey}/learn`);
      } else {
  ...
  const onLoginSuccess = useCallback(() => {
    ...
    if (redirectData.action === "enroll" && redirectData.courseId) {
      navigate(`/find-my-courses/${redirectData.courseId}/learn`);
    } else {
