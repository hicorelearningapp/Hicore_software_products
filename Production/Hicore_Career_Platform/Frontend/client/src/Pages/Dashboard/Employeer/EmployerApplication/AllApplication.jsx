...
//NewApplication Import all data files
import newData from "./NewApplicationData";
import shortlistedData from "./shortlistedData";
import inReviewData from "./InReviewData";
import interviewedData from "./InterviewedData";
import offerSentData from "./OfferSentData";
...
export default function AllApplication() {
  ...
  //NewApplication Collect all candidates from every data file
  const allSources = [
    newData,
    shortlistedData,
    inReviewData,
  ...
  const combinedCandidates = allSources.flatMap((src) =>
    src.candidates.map((c) => ({
      ...c,
      ribbonLabel: src.ribbonLabel, // attach ribbon label from source
    }))
  );

  const totalAllApplication = combinedCandidates.length;

  //NewApplication Apply filters
  ...
  return (
    <div className="p-6 min-h-screen text-gray-100">
      ...
      {/* Heading */}
      <h2 className="text-2xl text-left font-semibold text-[#4F80BF] mb-6">
        {totalAllApplication} All Applications
      </h2>
        ...
        {filtered.map((c) => (
          ...
          >
            ...
            <div className="absolute top-4 right-0">
              <span className="bg-[#4F80BF] rounded-tl-full rounded-bl-full text-[#FFFFFF] px-4 py-2 text-sm font-medium whitespace-nowrap">
                {c.ribbonLabel}
              </span>
            </div>
