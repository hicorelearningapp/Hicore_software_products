...
import completedSessionIcon from "../../../../assets/MentorSessions/Calendar.png";
import {
  CalendarDays,
  Clock,
  Users,
    ...
    const fetchSessions = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/mentor/${mentorId}/sessions/completed`
        );
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center w-full h-[300px]">
        ...
        <p className="text-gray-400 text-sm mb-6">
          Your completed sessions history will appear here
        </p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-7xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {sessions.map((session) => (
          ...
          >
            {/* Status */}
            <div className="bg-green-100 text-green-700 text-sm font-medium px-3 py-1 rounded-full w-max mb-3">
              Completed Session
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button className="flex-1 flex items-center justify-center gap-2 bg-[#3D2C8D] text-white rounded-md py-2 text-sm font-medium hover:bg-[#2c1f66] transition">
                <Play size={16} /> View Recording
              </button>
