...
const MentorDashboard = () => {
  ...
  // ✅ Sync breadcrumb with URL (if page is refreshed or navigated directly)
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="w-64 bg-white mb-2 border-r border-gray-300 rounded-xl shadow-xl p-4">
        <h2
          onClick={() => navigate("/")}
          className="flex items-center justify-center gap-2 text-sm font-bold text-[#343079] mb-6 cursor-pointer hover:text-[#343079] border-b-2 border-[#343079] pb-4"
        >
                  ...
                  className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer transition-colors group 
                    ${
                      activeMenu === item.label && !activeSubMenu
                        ...
                        : "text-indigo-900 hover:bg-[#343079] hover:text-white"
                    }`}
                  onClick={() => {
                    setActiveMenu(item.label);
                  ...
                  <ul className="ml-10 mt-2 space-y-2 text-sm">
                    {item.subMenu.map((sub, subIndex) => (
                      <li
                        ...
                        onClick={() => {
                          setActiveMenu(item.label);
                          setActiveSubMenu(sub.label); // ✅ store sub menu name
      ...
      <main className="flex-1 flex flex-col">
        ...
        <header className="flex justify-between items-center bg-white rounded-lg px-6 py-4 mb-0 ml-0 m-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#343079] font-semibold text-lg">
              Hello, Karthi Kumar
            </span>
          </div>
          ...
          <form
            ...
            className="flex items-center w-1/2 border border-gray-200 bg-gray-100 rounded-md px-3 py-2"
          >
