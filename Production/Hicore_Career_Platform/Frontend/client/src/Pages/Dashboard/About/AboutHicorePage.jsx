...
const AboutHiCorePage = () => {
  ...
  return (
    <section className="relative z-20 px-4 sm:px-6">
      ...
      <div className="absolute top-0 left-0  w-full h-[200px] bg-[#F9F9FC] -z-10" />

      {/* White curved box */}
      <div className="max-w-7xl mx-auto pt-4">
        <div className="bg-white rounded-3xl border border-blue-900 mb-6  p-6 sm:p-8 md:p-10 flex flex-col gap-10">
          {/* Header Section */}
          <div className="text-center">
            ...
            <p className="w-full mx-auto text-center font-poppins font-normal text-base leading-[32px] text-[#343079] md:max-w-[900px]">
              HiCore Career Project Platform empowers you to build real-world projects, gain certifications,
              connect with mentors, and launch your dream career{" "}
              <span className="text-[#343079] font-poppins font-semibold">— all in one place.</span>
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {helpItems.map(({ title, desc, img }, index) => (
                ...
                >
                  ...
                  <p className="w-full font-poppins font-normal text-[16px] leading-[32px] text-[#343079]">{desc}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <ul className="w-full space-y-3 pl-0 sm:pl-16">
                {features.map((feature, index) => (
                  <li key={index} className="w-full flex gap-2 items-start">
                    <img src={starIcon} alt="star" className="w-[24px] h-[24px] flex-shrink-0" />
                    <span className="font-poppins font-normal text-[16px] leading-[24px] text-[#343079]">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
...
export default AboutHiCorePage;
