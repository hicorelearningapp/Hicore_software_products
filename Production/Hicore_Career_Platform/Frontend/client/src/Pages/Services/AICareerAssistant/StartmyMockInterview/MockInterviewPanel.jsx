...
const fetchFeedback = async () => {
  ...
  try {
    const url = `${API_BASE}/ai/mock-interview/evaluate-qa`;

    const payload = {
      transcript,
      topic: selectedQuestion?.question || "",
      context: jobRole,
      audioConfidence: 0.94,
    };
    ...
    const apiData = data?.evaluation || {};

    // ✅ Convert API scores object into UI-friendly format
    const formattedScores = {};
    if (apiData.scores) {
      Object.keys(apiData.scores).forEach((key) => {
        formattedScores[key.replaceAll("_", " ").toUpperCase()] = {
          score: apiData.scores[key].score,
          feedback: apiData.scores[key].feedback,
        };
      });
    }

    const formattedFeedback = {
      overall: apiData.summary || "",
    ...
    alert("AI server not reachable or wrong API path.");
  } finally {
      ...
      <div className="bg-white border border-[#E2E1F3] rounded-xl shadow-sm w-full p-4 sm:p-6">
        {!recordMode ? (
          <>
           ...
           <div className="flex flex-wrap gap-2 sm:gap-6 mb-2 pb-2">
           <label className="text-[16px] font-semibold  py-3 rounded-md 
                text-[#3D3584]">
               Questions
           </label>
         </div>
