...
const Addnewdevice = ({ onClose }) => {
  ...
  const itemOptions = [
    { id: 101, display: "ITM-001 (Laptop)" },
    { id: 102, display: "ITM-002 (Monitor)" },
    { id: 103, display: "ITM-003 (Keyboard)" }
  ];
  ...
  // Basic validation for the fields we actually show in the UI
  if (!deviceName) {
  ...
  // This payload exactly matches the schema you provided
  const payload = {
    DeviceName: deviceName,
    DeviceType: deviceType, // "Scale", "Sensor", etc.
    ConnectionMode: "WiFi", // Default
    Capacity: 0,            // Required by schema
    Weight: 0,              // Required by schema
    LastReading: 0,         // Required by schema
    Battery: 100,           // Required by schema
    Status: "Online",       // Required by schema
    InventoryId: linkToItem && selectedItem ? Number(selectedItem.id) : 0,
    Notes: notes || "",     // Optional but kept in schema
    LocationName: location || "Unassigned",
    Latitude: 0,            // Required by schema
    Longitude: 0            // Required by schema
  };

  try {
    const response = await axios.post(`${API_BASE}/devices`, payload);
    
    // Most APIs return a 201 Created or a success flag
    ...
    if (error.response && error.response.data) {
      ...
      alert("Failed to create device. Please check your connection.");
    }
  } finally {
    ...
    <div className="relative w-full flex flex-col gap-[24px] md:gap-[36px] p-[20px] md:p-[36px] rounded-[24px] md:rounded-[36px] bg-white max-h-[90vh]">
        ...
        <div className="flex flex-col gap-[20px]">
          ...
          <div className="flex flex-col gap-[12px] relative">
            <label className="flex items-center gap-[8px] text-[14px] text-[#0A2A43] cursor-pointer w-max">
              <input type="checkbox" checked={linkToItem} onChange={(e) => { setLinkToItem(e.target.checked); if(!e.target.checked) setSelectedItem(null); }} className="w-[16px] h-[16px] accent-[#0A2A43]" />
              Link to Item Code
            </label>

            {linkToItem && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-200" ref={itemRef}>
                <label className="text-[14px] text-[#0A2A43]">Select Item</label>
                <div onClick={() => setItemDropdownOpen(!itemDropdownOpen)} className="flex items-center justify-between px-[16px] py-[10px] rounded-[80px] border border-[#8A939B] cursor-pointer bg-white">
                  <span className={`text-[14px] ${selectedItem ? "text-[#0A2A43]" : "text-[#8A939B]"}`}>
                    {selectedItem ? selectedItem.display : "Select Item"}
                  </span>
                  <img src={downarrow} alt="arrow" className={`w-[20px] h-[20px] transition-transform ${itemDropdownOpen ? "rotate-180" : ""}`} />
                </div>
                {itemDropdownOpen && (
                  <div className="absolute left-0 right-0 mt-2 z-20 rounded-[16px] border border-[#E7EAEC] bg-white shadow-xl max-h-[160px] overflow-y-auto no-scrollbar">
                    {itemOptions.map((item) => (
                      <div key={item.id} onClick={() => { setSelectedItem(item); setItemDropdownOpen(false); }} className="px-[16px] py-[12px] text-[14px] text-[#0A2A43] hover:bg-[#F5F7FA] cursor-pointer border-b last:border-0 border-[#E7EAEC]/50">
                        {item.display}
                      </div>
                    ))}
                  </div>
      ...
      <div className="w-full flex flex-col sm:flex-row justify-between gap-[12px]">
        <button
          ...
          className="w-full sm:w-[50%] h-[44px] rounded-[80px] bg-[#1769FF] text-white text-[14px] font-medium transition-opacity disabled:opacity-50"
        >
          {isSubmitting ? "Processing..." : "Add Device"}
        </button>
        <button
          onClick={onClose}
        ...
        <div className="absolute inset-0 z-[100] bg-white/95 flex items-center justify-center rounded-[24px] md:rounded-[36px] p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-[390px] flex flex-col gap-[16px]">
            <div className="flex justify-end">
              <img src={closeicon} alt="close" className="w-[24px] h-[24px] cursor-pointer" onClick={onClose} />
            </div>
