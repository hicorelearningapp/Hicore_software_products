...
import { FaFilePdf, FaCheckCircle, FaRegClock, FaImage } from "react-icons/fa";
...
const ExtractImages = () => {
  ...
  const handleExtractImages = async () => {
    ...
    try {
      // STEP 1: Ask backend for upload endpoint (UNCHANGED)
      ...
      if (!actionRes.ok) {
        throw new Error("Failed to initiate extract images action");
      }
      ...
      // STEP 2: Upload PDF (same fields, unchanged)
      const fd = new FormData();
      fd.append("files", file); // backend still expects `files`
      fd.append("tool", "organize");
      fd.append("action", "extract_images");
      ...
      // STEP 3: ZIP response (important)
      const blob = await res.blob();

      // Optional safety check
      if (blob.type !== "application/zip") {
        console.warn("Response is not ZIP:", blob.type);
      }
          ...
          <div className="col-span-9 relative">
            <div className="flex items-center justify-start bg-[#F7E6E9]/[0.30] border border-[#F3D9DD] rounded-md px-4 py-6">
              <div className="flex items-center gap-10">
                <button
                  onClick={() => navigate("/tools/extract-images")}
              ...
              {resultUrl ? (
                ...
                <div className="flex flex-col items-center text-gray-400">
                  ...
                  <p className="text-xs px-4 text-center leading-relaxed">
                    {isProcessing
                      ? "Processing your PDF..."
                      : "Click the button to extract all images."}
                  </p>
                </div>
              )}
              {error && (
              ...
              onClick={
                resultUrl
                  ? () => {
                      ...
                      // ✅ FORCE ZIP — do NOT reuse PDF filename
              ...
              <div className="mt-5 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="border border-[#E7B0B9] bg-[#F7E6E9]/[30%] rounded-md p-4">
                  <p className="text-sm font-semibold mb-2 flex items-center gap-2">
                    <FaImage className="text-red-700" /> All Images
                  </p>
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    This tool will intelligently scan your PDF and extract every
                    individual image file found within it.
                  </p>
                </div>
                ...
                <div className="mt-8 pt-5 border-t border-[#E7B0B9]/50">
                  <h4 className="flex items-center gap-2 text-[14px] font-bold text-red-800 uppercase mb-3">
                    <LuLightbulb className="text-yellow-600" /> Information
                  </h4>
                  <ul className="space-y-3 text-[13px] text-gray-600">
                    <li className="flex gap-2">
                      <span>•</span>{" "}
                      <span>Original image quality is maintained.</span>
                    </li>
                    <li className="flex gap-2">
                      <span>•</span>{" "}
                      <span>Ideal for extracting high-res photos.</span>
                    </li>
                  </ul>
                </div>
              </div>
