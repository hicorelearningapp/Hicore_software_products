// src/pages/tools/CompressPdf.jsx
...
const presetQualities = [
  { label: "High (low compression)", value: 90 },
  { label: "Medium", value: 60 },
  { label: "Low (high compression)", value: 30 },
];
...
const CompressPdf = () => {
  ...
  const parseTargetSize = (s) => {
    if (!s || !s.trim()) return null;
    const raw = s.trim().toLowerCase();
    const m = raw.match(/^(\d+(\.\d+)?)(\s*(kb|mb))?$/i);
    if (!m) return null;
  ...
  const handleCompress = async () => {
    ...
    try {
      const actionRes = await fetch(`${API_BASE}/api/tool-action`, {
        ...
        body: JSON.stringify({ tool: "organize", action: "compress_pdf" }),
      });
      ...
      fd.append("action", "compress_pdf");
      fd.append("quality", String(quality));

      const settings = { quality };
      const bytes = parseTargetSize(targetSizeInput);
      if (bytes) {
        fd.append("target_size_bytes", String(bytes));
        settings.target_size_bytes = bytes;
      }
        ...
        <div className="grid grid-cols-12 gap-6 mb-4">
              ...
              <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-3">
                {[iconMonitor, iconGrid, iconTriangle, iconCloud].map(
                  (img, i) => (
                    <button
                      key={i}
                      className="p-2 bg-white border border-[#B2011E] rounded-md hover:bg-red-50"
                    >
              ...
              {resultUrl ? (
                ...
                <div className="flex flex-col items-center text-gray-400">
                  ...
                  <p className="text-xs px-4 text-center">
                    {isProcessing ? "Optimizing PDF..." : "Ready to compress."}
                  </p>
                </div>
              )}
              {error && (
                <p className="text-red-500 text-[10px] mt-4 text-center">
                  {error}
                </p>
              )}
            </div>
              ...
              onClick={
                resultUrl
                  ? () => {
                      ...
                      a.href = resultUrl;
                      a.download = `compressed_${file?.name || "result.pdf"}`;
                      a.click();
                    }
                  : handleCompress
              }
          ...
          <div className="col-span-3">
            <aside className="bg-gray-50 rounded-lg p-4 h-[75vh] flex flex-col">
              <div className="flex items-center justify-between border border-[#E7B0B9] bg-[#F7E6E9]/[0.30] p-3 rounded-md">
                {file ? (
                ...
                {file && (
                  <button
                    ...
                    className="flex items-center gap-2 text-red-600 hover:text-red-800 p-1"
                  >
                    <FiTrash2 size={18} />
              ...
              <div className="mt-5 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="border border-[#E7B0B9] bg-[#F7E6E9]/[30%] rounded-md p-4 space-y-4">
                  <div>
                    <label className="text-xs font-bold block mb-2 uppercase">
                      Quality
                    </label>
                    <select
                      value={quality}
                      onChange={(e) => setQuality(Number(e.target.value))}
                      className="w-full px-2 py-2 border border-[#E7B0B9] bg-white rounded text-sm focus:outline-none"
                    >
                      {presetQualities.map((p) => (
                ...
                <div className="mt-8 pt-5 border-t border-[#E7B0B9]/50">
                  <h4 className="flex items-center gap-2 text-[14px] font-bold text-red-800 uppercase mb-3">
                    <LuLightbulb className="text-yellow-600" /> Tips
                  </h4>
                  <ul className="space-y-3 text-[13px] text-gray-600">
                    <li>• Lower quality = smaller file size.</li>
                    <li>• Target size is a goal, not a guarantee.</li>
                  </ul>
                </div>
              </div>
            </aside>
          ...
          <main className="col-span-6 bg-white border border-[#E7B0B9] rounded-lg p-4 flex flex-col shadow-sm">
            {file ? (
              <div className="flex flex-col h-[75vh]">
                ...
                <div className="flex justify-between items-center mt-4 px-2">
                  <button
                    ...
                    className="px-6 py-1.5 border border-red-200 rounded text-[10px] font-black text-red-700 uppercase hover:bg-red-50 disabled:opacity-30"
                  >
                    Prev
                  </button>
              ...
              <div className="flex flex-col items-center justify-center h-[70vh] text-gray-300">
                ...
                <p className="font-bold text-gray-400">Preview Area</p>
              </div>
            )}
          </main>
        </div>
