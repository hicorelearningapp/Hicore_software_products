// src/pages/tools/OrganizePages.jsx
...
const OrganizePages = () => {
  ...
  const handleOrganize = async () => {
    ...
    if (!pageOrder.trim()) return alert("Please enter the new page order.");
    ...
    try {
      const actionRes = await fetch(`${API_BASE}/api/tool-action`, {
        ...
        body: JSON.stringify({ tool: "organize", action: "reorder_pdf" }),
      });
      ...
      fd.append("action", "reorder_pdf");
      fd.append("settings", JSON.stringify({ order: pageOrder }));
          ...
          <div className="col-span-9 relative">
            <div className="flex items-center justify-start bg-[#F7E6E9]/[0.30] border border-[#F3D9DD] rounded-md px-4 py-6">
              <div className="flex items-center gap-10">
                <button
                  onClick={() => navigate("/tools/organize-pages")}
              ...
              <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-3">
                {[iconMonitor, iconGrid, iconTriangle, iconCloud].map(
                  (img, i) => (
                    <button
                      key={i}
                      className="p-2 bg-white border border-[#B2011E] rounded-md transition-colors hover:bg-red-50"
                    >
              ...
              {resultUrl ? (
                ...
                <div className="flex flex-col items-center text-gray-400">
                  ...
                  <p className="text-xs px-4 text-center leading-relaxed">
                    {isProcessing
                      ? "Processing your PDF..."
                      : "Set the page order and click Organize."}
                  </p>
                </div>
              )}
              {error && (
                <p className="text-red-500 text-[10px] mt-4 text-center font-medium">
                  {error}
                </p>
              )}
            </div>
              ...
              onClick={
                resultUrl
                  ? () => {
                      ...
                      a.href = resultUrl;
                      a.download = `organized_${file?.name || "result.pdf"}`;
                      a.click();
                    }
                  : handleOrganize
              }
          ...
          <div className="col-span-3">
            <aside className="bg-gray-50 rounded-lg p-4 h-[75vh] flex flex-col">
              ...
              <div className="mt-5 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="border border-[#E7B0B9] bg-[#F7E6E9]/[30%] rounded-md p-4">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#E7B0B9] bg-white rounded text-sm mb-2 focus:outline-none focus:ring-1 focus:ring-red-400"
                  />
                  <p className="text-[11px] text-gray-500">
                    Required: specific order of pages.
                  </p>
                </div>
                ...
                <div className="mt-8 pt-5 border-t border-[#E7B0B9]/50">
                  <h4 className="flex items-center gap-2 text-[14px] font-bold text-red-800 uppercase mb-3">
                    <LuLightbulb className="text-yellow-600" /> Guide
                  </h4>
                  <ul className="space-y-3 text-[13px] text-gray-600">
                    <li className="flex gap-2">
                      <span>•</span>{" "}
                      <span>Comma separated numbers (3,1,2)</span>
                    </li>
                    <li className="flex gap-2">
                      <span>•</span>{" "}
                      <span>Duplicates are automatically removed.</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="flex flex-col h-[75vh]">
                ...
                <div className="flex justify-between items-center mt-4 px-2">
                  <button
                    ...
                    className="px-6 py-1.5 border border-red-200 rounded text-[10px] font-black disabled:opacity-30 text-red-700 uppercase tracking-widest hover:bg-red-50 transition-colors"
                  >
                    Prev
                  </button>
